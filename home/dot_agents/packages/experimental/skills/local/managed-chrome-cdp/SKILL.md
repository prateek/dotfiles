...
- Script: `~/.agents/skills/managed-chrome-cdp/scripts/browser-tools.ts`
...
- Start a headless CDP Chrome using a persistent profile dir:
  - `bun ~/.agents/skills/managed-chrome-cdp/scripts/browser-tools.ts start --headless --port 9333`
...
- Check if automation can access the portal:
  - `bun ~/.agents/skills/managed-chrome-cdp/scripts/browser-tools.ts ensure-auth "<url>" --port 9333`
- If it says login is required:
  - `bun ~/.agents/skills/managed-chrome-cdp/scripts/browser-tools.ts login "<url>" --port 9333`
...
- If the profile gets wedged/corrupted (or CDP won’t come up), reset it:
  - `bun ~/.agents/skills/managed-chrome-cdp/scripts/browser-tools.ts reset-profile --force`
