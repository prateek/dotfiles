...
Full architectural rationale is in `dev/adr/0001-downstream-fork-architecture.md` in the dotfiles repo. Plan doc with concrete workflow recipes is at `dev/docs/setup-downstream-fork-plan.md`. Read `references/architecture.md` in this skill for an LLM-oriented summary if you need to extend the skill itself.
