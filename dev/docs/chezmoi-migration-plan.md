...
| `BetterTouchTool preferences` | readable source config in `home/.chezmoiassets/Library/Preferences/com.hegenberg.BetterTouchTool.json`; `home/Library/Preferences/modify_private_com.hegenberg.BetterTouchTool.plist.tmpl` merges selected app preferences only. Trigger stores, clipboard databases, usage, license, remote-device, and window state stay local or in app sync. | 3 | implemented |
| `Raycast preferences` | readable source config in `home/.chezmoiassets/Library/Preferences/com.raycast.macos.json`; `home/Library/Preferences/modify_private_com.raycast.macos.plist.tmpl` merges selected UI/search preferences only. Cloud sync records, encrypted databases, notes IDs, analytics IDs, window state, account state, and extension data stay local or in Raycast sync. | 3 | implemented |
| `Tailscale preferences` | readable source config in `home/.chezmoiassets/Library/Preferences/io.tailscale.ipn.macsys.json`; `home/Library/Preferences/modify_private_io.tailscale.ipn.macsys.plist.tmpl` merges selected app UI/startup preferences only. Cached profiles, restart state, VPN runtime state, and account data stay local. | 3 | implemented |
| `Setapp preferences` | readable source config in `home/.chezmoiassets/Library/Preferences/com.setapp.DesktopClient.json`; `home/Library/Preferences/modify_private_com.setapp.DesktopClient.plist.tmpl` merges selected UI/update preferences only. Account, subscription, app catalog, notification token, analytics, and installed Setapp-app state stay local. | 3 | implemented |
| `BetterDisplay preferences` | readable source config in `home/.chezmoiassets/Library/Preferences/pro.betterdisplay.BetterDisplay.json`; `home/Library/Preferences/modify_private_pro.betterdisplay.BetterDisplay.plist.tmpl` merges global menu/update preferences only. Display identifiers, display topology, color profile paths, license files, and per-monitor state stay local. | 3 | implemented |
