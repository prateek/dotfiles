...
description: Single entry point for chezmoi work in Prateek's dotfiles repo. Use when running `chezmoi apply`/`diff`/`verify`/`merge`/`edit`/`re-add`/`forget`; resolving target-vs-source drift; editing or capturing files under `home/.chezmoitemplates/`, `home/.chezmoiassets/`, `home/.chezmoiscripts/`, `home/.chezmoidata/`, or `home/.chezmoiexternal.*`; rendering `brewfile.tmpl`; toggling chezmoi-owned `DOTFILES_*` env vars (init prompts `INSTALL_PROFILE`/`RUN_INSTALL_SCRIPTS`/`APPLY_DEFAULTS`/`SECRETS_ENABLED`, env-set-at-init `INSTALL_XCODE`/`MANAGE_ZINIT_EXTERNAL`, and apply-time gates for MAS, Homebrew tuning, plist hooks, post-apply relaunch — full table in `references/packages-and-secrets.md`); wiring 1Password `op://` references; or translating between `~/.<file>` paths and `home/<dot_*>` source paths. Do not use for unrelated dotfile edits that do not touch chezmoi state, shell-startup debugging that does not change source files, generic "manage my dotfiles" requests with no chezmoi command or `home/.chezmoi*/` path involved, or test/VM/harness vars (`DOTFILES_TART_*`, `DOTFILES_WARM_*`, `DOTFILES_TRACE*`, `DOTFILES_AUDIT_*`, `DOTFILES_CAPTURE_*`, `DOTFILES_SKIP_LAUNCHCTL_SYNC`, `DOTFILES_ROOT`, `DOTFILES_SUDO_*`).
---
