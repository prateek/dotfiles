...
def resolve_default_sim_skill_dir(audit_skill_dir: Path) -> Path:
    candidates = [audit_skill_dir.parent / "ios-simulator-skill"]
    project = next((parent for parent in audit_skill_dir.parents if parent.name == "agent-marketplace"), None)
    if project:
        candidates.append(project / "build/marketplace/plugins/ios/skills/ios-simulator-skill")
    return next(
