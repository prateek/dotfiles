...
class RunWorkflowsTests(unittest.TestCase):
    ...
    def test_resolves_simulator_skill_from_package_source(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            project = Path(tmp) / "agent-marketplace"
            audit_root = project / "packages/ios/skills/ios-audit"
            simulator_root = project / "build/marketplace/plugins/ios/skills/ios-simulator-skill"
            audit_root.mkdir(parents=True)
