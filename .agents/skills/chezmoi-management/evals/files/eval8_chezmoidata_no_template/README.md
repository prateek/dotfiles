...
Simulated state: existing `home/.chezmoidata/packages.toml` with two profiles.
...
- Agent offers at least one correct alternative:
  - Per-profile separation via `DOTFILES_INSTALL_PROFILE` (matches existing repo pattern).
