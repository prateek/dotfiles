...
* **transcript**: unstructured text with speaker turns (may be imperfect, may have `Speaker 1: ...` / `John:` / or no speakers).
* **optional context**: meeting title, team, date, attendees, or agenda (if present in the transcript, extract it).
...
```markdown
# Meeting Notes
**Title:** {{infer from transcript or write "Untitled"}}  
**Date:** {{infer or write "Unknown"}}  
**Attendees:** {{list names if present, else "Not specified"}}  
**Facilitator/Primary speaker:** {{infer or "Not specified"}}
...
Produce **only** the Markdown output above. Do **not** explain what you did. Fill in all `TBD`s where needed.
