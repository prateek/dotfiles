...
| `osx-apps/cmux/` | desired-plist fragment at `home/.chezmoitemplates/com.cmuxterm.app.plist.tmpl`; `home/Library/private_Preferences/modify_private_com.cmuxterm.app.plist.tmpl` is a 3-line stub that runs the shared engine. Reusable icon asset stays under `scripts/macos/assets/cmux/`. Session state, browser history, PostHog cache, and icon backups stay local. | 3 |implemented (shared engine, commit `b2b43de`) |
