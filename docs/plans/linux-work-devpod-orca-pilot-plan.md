...
DAYJOB cloud development environment without changing the macOS work profile
or making the behavior the default for another user.
...
- Leave image packages, runtimes, Git identity, and direct shared agent assets
  under DAYJOB ownership. Dotfiles owns its user-local rendering
  prerequisites.
- Keep the pilot opt-in through private per-user DAYJOB configuration.
- Use the released checksum-pinned Debian package and a user tmux session.
- Use the existing DAYJOB tunnel and Orca authentication boundary.
...
DAYJOB provides two landed private layers:

- Common configurable login-shell support and a bounded per-user startup hook.
- Prateek's default-off hook that updates `~/dotfiles`, applies
  `machine_type=work`, and starts or stops Orca.
...
DAYJOB has written its own keys. DAYJOB remains the sole writer of the
...
DAYJOB per-user hook ────────────────> dotfiles apply + Orca reconcile ──┤
                                                                        │
...
| DAYJOB common startup-hook and login-shell support | Landed |
| DAYJOB Prateek-specific desired-state hook | Landed |
| Linux composite profile and image-ownership ignores | Implemented locally |
| Plugin projection, reconciler hardening, pairing command, and Ubuntu lane | Implemented locally |
...
3. DAYJOB-owned Claude asset trees remain untouched, its settings keys
...
7. A workstation cold start converges automatically, reconnects through the
   private DAYJOB tunnel, and opens an existing paired Orca environment.

## Risks and deferred work

- Port 6768 is not tunnel-exclusive. The pilot accepts the existing DAYJOB
  access controls plus Orca authentication boundary; public ingress remains
  out of scope.
...
- The exact user hook is intentionally personal. Generalizing Orca lifecycle
  into shared DAYJOB infrastructure requires a separate adoption decision.
