...
Accepted ADRs are stricter than the generic transition graph: once accepted,
they can remain `accepted` or close as `superseded` / `archived`, but they
should not reopen to `active` or become `current` guidance. Create a new ADR or
reference/runbook instead.
...
| Type | Allowed statuses | Use when | Do not use when |
| --- | --- | --- | --- |
...
- Keep `docs/` Markdown-only except for `docs/validate-doc-lifecycle.py`.
  Generated captures, screenshots, and parser byproducts belong outside `docs/`
  unless they are gitignored.
