...
- `smoke` runs the chezmoi-blessed bootstrap (`sh -c "$(curl -fsLS get.chezmoi.io)" -- init --apply`) with `DOTFILES_INSTALL_PROFILE=core` in a clean macOS guest and is the default real-VM lane. (Originally invoked `install.sh --core`; that script was retired in favor of the chezmoi-native bootstrap — see ADR 0006 update 2026-05-02.)
