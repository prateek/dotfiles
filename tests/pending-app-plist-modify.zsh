...
for app_spec in "${apps[@]}"; do
  app_name="${app_spec%%:*}"
  domain="${app_spec#*:}"
  source_json="$DOTFILES_ROOT/home/.chezmoiassets/Library/Preferences/$domain.json"
...
assets_root = dotfiles_root / "home/.chezmoiassets/Library/Preferences"
...
for app_name, spec in cases.items():
    ...
    desired = plistlib.loads(
        plistlib.dumps(__import__("json").loads((assets_root / f"{domain}.json").read_text()))
    )
