...
render_script() {
  ...
  env -u DOTFILES_MACHINE_TYPE chezmoi \
    ...
    --override-data "{\"run_install_scripts\":true,\"apply_macos_defaults\":false,\"secrets_enabled\":false,\"machine_type\":\"$machine_type\",\"install_xcode\":$install_xcode,\"manage_zinit_external\":false}" \
    execute-template \
...
render_script personal "$personal_script" true
bash -n "$personal_script" || die "rendered personal script has invalid syntax"

personal_skip_script="$tmp_root/xcode-personal-skip.sh"
render_script personal "$personal_skip_script" false
bash -n "$personal_skip_script" || die "rendered personal skip script has invalid syntax"
