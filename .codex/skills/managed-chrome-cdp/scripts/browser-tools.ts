 ...
 *
 * Adapted from: https://github.com/steipete/agent-scripts/blob/main/scripts/browser-tools.ts
 *
...
const BOOTSTRAP_ENV = 'BROWSER_TOOLS_BOOTSTRAPPED';
const BOOTSTRAP_CACHE_ENV = 'BROWSER_TOOLS_CACHE_DIR';
const ORIGINAL_ARGV0_ENV = 'BROWSER_TOOLS_ORIGINAL_ARGV0';
const ORIGINAL_ARGV1_ENV = 'BROWSER_TOOLS_ORIGINAL_ARGV1';

const SELF_CONTAINED_DEPS: Record<string, string> = {
  commander: '14.0.2',
  'puppeteer-core': '24.34.0',
};

function cacheRootDir(): string {
  if (process.env[BOOTSTRAP_CACHE_ENV]) return String(process.env[BOOTSTRAP_CACHE_ENV]);
  // Prefer keeping this skill self-contained: install deps into a cache folder adjacent to this script.
  // Falls back to the standard per-user cache location if the skill directory is not writable.
  const localCacheRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), '.cache');
  try {
    mkdirSync(localCacheRoot, { recursive: true });
    return localCacheRoot;
  } catch {
    // ignore and fall back to system cache
  }
  if (process.env.XDG_CACHE_HOME) return String(process.env.XDG_CACHE_HOME);
  const home = os.homedir();
  if (process.platform === 'darwin') return path.join(home, 'Library', 'Caches');
  return path.join(home, '.cache');
}

function bootstrapCacheDir(): string {
  const bunVersion = (process.versions as { bun?: string }).bun ?? 'unknown';
  const key = createHash('sha256')
    .update(JSON.stringify({ deps: SELF_CONTAINED_DEPS, bun: bunVersion }))
    .digest('hex')
    .slice(0, 12);
  return path.join(cacheRootDir(), 'codex', 'browser-tools', key);
}

function hasInstalledDeps(dir: string): boolean {
  return (
    existsSync(path.join(dir, 'node_modules', 'commander', 'package.json')) &&
    existsSync(path.join(dir, 'node_modules', 'puppeteer-core', 'package.json'))
  );
}

function ensureCachePackageJson(dir: string) {
  const packageJsonPath = path.join(dir, 'package.json');
  const content = JSON.stringify(
    {
      name: 'browser-tools-cache',
      private: true,
      type: 'module',
      dependencies: SELF_CONTAINED_DEPS,
    },
    null,
    2,
  );

  const existing = existsSync(packageJsonPath) ? readFileSync(packageJsonPath, 'utf8') : '';
  if (existing.trim() === content.trim()) return;
  writeFileSync(packageJsonPath, `${content}\n`, 'utf8');
}

function installDeps(dir: string) {
  const result = spawnSync('bun', ['install', '--no-progress'], { cwd: dir, stdio: 'inherit' });
  if (result.status !== 0) {
...
function bootstrapIfNeeded() {
  ...
  if (!(process.versions as { bun?: string }).bun) {
    console.error('✗ This script currently requires Bun. Install Bun (https://bun.sh) and re-run with `bun`.');
    process.exit(1);
  }

  const sourcePath = fileURLToPath(import.meta.url);
  const cacheDir = bootstrapCacheDir();
  ...
  if (!hasInstalledDeps(cacheDir)) {
    installDeps(cacheDir);
  }

  copyFileSync(sourcePath, cachedPath);

  const result = spawnSync('bun', [cachedPath, ...process.argv.slice(2)], {
    cwd: process.cwd(),
    stdio: 'inherit',
    env: {
      ...process.env,
      [BOOTSTRAP_ENV]: '1',
      [ORIGINAL_ARGV0_ENV]: process.env[ORIGINAL_ARGV0_ENV] ?? process.argv[0],
      [ORIGINAL_ARGV1_ENV]: process.env[ORIGINAL_ARGV1_ENV] ?? sourcePath,
    },
  });
...
let CommanderCommand: any;
let puppeteer: any;

async function loadDeps() {
  if (!CommanderCommand) {
    const commanderModule = await import('commander');
    CommanderCommand = (commanderModule as any).Command ?? commanderModule.default?.Command;
  }
  if (!puppeteer) {
    const puppeteerModule = await import('puppeteer-core');
    puppeteer = (puppeteerModule as any).default ?? puppeteerModule;
  }
}
...
function invocationBase(): string {
  const argv0 = process.env[ORIGINAL_ARGV0_ENV] ?? process.argv[0];
  const argv1 = process.env[ORIGINAL_ARGV1_ENV] ?? process.argv[1];
  if (!argv1) return shellQuote(argv0);
  return `${shellQuote(argv0)} ${shellQuote(argv1)}`;
}
...
function resolveChromePath(chromePath: string): string {
  const expanded = expandHomePath(chromePath);
...
function validateChromePath(chromePath: string): string {
  ...
  if (!existsSync(resolved)) {
    ...
    if (process.platform === 'darwin') {
      console.error(`  Default: ${DEFAULT_CHROME_BIN}`);
    }
    process.exit(1);
  }
  return resolved;
...
function resolveProfileDir(profileDir: string): string {
  return path.resolve(expandHomePath(profileDir));
}

function assertSafeProfileDir(profileDir: string): string {
  const resolved = resolveProfileDir(profileDir);
  const root = path.parse(resolved).root;
  if (resolved === root) {
    throw new Error(`Refusing to use profile dir ${resolved} (filesystem root).`);
  }
  if (resolved === path.resolve(os.homedir())) {
    throw new Error(`Refusing to use profile dir ${resolved} (home directory).`);
  }
  return resolved;
}

function safeProfileDirOrExit(profileDir: string): string {
  try {
    return assertSafeProfileDir(profileDir);
  } catch (error) {
...
async function waitForBrowser(port: number, timeoutMs: number): Promise<boolean> {
...
function spawnDevtoolsChrome(options: {
  chromePath: string;
  port: number;
  profileDir: string;
  headless: boolean;
  ...
  const { chromePath, port, profileDir, headless, detached } = options;
  const chromeArgs = [
  ...
  if (headless) {
    chromeArgs.push('--headless=new', 'about:blank');
  }

  const proc = spawn(chromePath, chromeArgs, { detached, stdio: 'ignore' });
...
async function ensureGitIgnored(profileDir: string) {
  if (path.basename(profileDir) !== 'chrome-cdp-profile' || path.basename(path.dirname(profileDir)) !== '.dev') {
    return;
  }
  ...
  const candidates = [path.join(gitRoot, '.gitignore'), path.join(gitRoot, '.git', 'info', 'exclude')];
  for (const filePath of candidates) {
    ...
    const alreadyPresent = content
      .split('\n')
      .map((line) => line.trim())
      .includes(DEFAULT_PROFILE_GITIGNORE_PATTERN);
    if (alreadyPresent) continue;

    const prefix = content.length > 0 && !content.endsWith('\n') ? '\n' : '';
    try {
      await appendFile(filePath, `${prefix}${DEFAULT_PROFILE_GITIGNORE_PATTERN}\n`, 'utf8');
    } catch {
...
function isPolicyCached(profileDir: string): boolean {
  return existsSync(path.join(profileDir, POLICY_CACHE_MARKER));
}

async function prewarmProfile(profileDir: string, chromePath: string, timeoutMs: number) {
  ...
  try {
    ...
    throw new Error(`Timed out waiting for policy cache (${POLICY_CACHE_MARKER}) in ${profileDir}`);
  } finally {
    try {
      proc.kill('SIGTERM');
    } catch {
...
function authStatePath(profileDir: string) {
  return path.join(profileDir, '.browser-tools', 'auth.json');
}

async function readAuthState(profileDir: string): Promise<AuthState> {
  const filePath = authStatePath(profileDir);
  try {
...
async function writeAuthState(profileDir: string, state: AuthState) {
  const filePath = authStatePath(profileDir);
  await mkdir(path.dirname(filePath), { recursive: true });
  await writeFile(filePath, JSON.stringify(state, null, 2), 'utf8');
}

async function markHostVerified(profileDir: string, host: string) {
  const state = await readAuthState(profileDir);
  state.verifiedHosts ??= {};
  state.verifiedHosts[host] = { lastVerified: new Date().toISOString() };
  await writeAuthState(profileDir, state);
}

function looksLikeAuthUrl(url: URL): boolean {
  const host = url.hostname.toLowerCase();
  const pathname = url.pathname.toLowerCase();

  const knownAuthHosts = new Set([
    'login.microsoftonline.com',
    'login.live.com',
    'accounts.google.com',
    'auth.datadoghq.com',
  ]);
  if (knownAuthHosts.has(host)) return true;
  if (host.includes('okta') || host.includes('auth0') || host.includes('onelogin')) return true;

  return ['login', 'signin', 'sso', 'oauth', 'authorize'].some((needle) => pathname.includes(needle));
}

async function needsLoginForUrl(page: any, targetUrl: URL): Promise<{ needsLogin: boolean; finalUrl: string }> {
  ...
  try {
    ...
    if (passwordInput) {
      return { needsLogin: true, finalUrl };
    }
  } catch {
  ...
  if (parsed) {
    if (parsed.hostname !== targetUrl.hostname && looksLikeAuthUrl(parsed)) {
      return { needsLogin: true, finalUrl };
    }
    if (parsed.hostname === targetUrl.hostname && looksLikeAuthUrl(parsed)) {
      return { needsLogin: true, finalUrl };
  ...
  return { needsLogin: false, finalUrl };
}
...
program
  ...
  .option('--profile', 'Copy your default Chrome profile before launch (best-effort).', false)
  ...
  .option('--headless', 'Run Chrome headless (no UI).', false)
  .option('--prewarm-timeout <seconds>', 'Policy prewarm timeout in seconds (default: 20).', (value) => Number.parseInt(value, 10), 20)
  .option('--reset-profile', 'Delete the profile directory before launch.', false)
  .option('--force', 'Allow destructive operations (e.g. --reset-profile).', false)
  .option('--kill-existing', 'Stop any running Google Chrome before launch (default: false).', false)
  .action(async (options) => {
    const { port, profile, profileDir, chromePath, headless, prewarmTimeout, resetProfile, force, killExisting } = options as {
      port: number;
      profile: boolean;
      profileDir: string;
      chromePath: string;
      headless: boolean;
      prewarmTimeout: number;
      resetProfile: boolean;
      force: boolean;
      killExisting: boolean;
    };

    const resolvedProfileDir = safeProfileDirOrExit(profileDir);
    const resolvedChromePath = validateChromePath(chromePath);
    ...
    if (resetProfile) {
      if (!force) {
        console.error('✗ --reset-profile is destructive; re-run with --force to confirm.');
        process.exit(1);
      }
    ...
    console.log(
      `✓ Chrome listening on http://localhost:${port}${headless ? ' (headless)' : ''}${profile ? ' (profile copied)' : ''}`,
    );
  });

program
  .command('reset-profile')
  .description('Delete the Chrome CDP profile directory (clears cookies and saved sessions).')
  .option('--profile-dir <path>', 'Chrome user-data-dir to delete.', DEFAULT_PROFILE_DIR)
  .option('--force', 'Skip the confirmation prompt.', false)
  .action(async (options) => {
    const profileDir = options.profileDir as string;
    if (!options.force) {
      const rl = readline.createInterface({ input, output });
      const answer = (await rl.question(`Delete ${profileDir}? This removes cookies/sessions. [y/N] `)).trim().toLowerCase();
      rl.close();
...
program
  .command('login <url>')
  .description('Open a URL in a headed Chrome session so you can log in, then (by default) restart headless.')
  .option('--port <number>', 'Debugger port (default: 9222)', (value) => Number.parseInt(value, 10), DEFAULT_PORT)
  .option('--profile-dir <path>', 'Chrome user-data-dir to use.', DEFAULT_PROFILE_DIR)
  .option('--chrome-path <path>', 'Path to the Chrome binary.', DEFAULT_CHROME_BIN)
  .option('--prewarm-timeout <seconds>', 'Policy prewarm timeout in seconds (default: 20).', (value) => Number.parseInt(value, 10), 20)
  .option('--keep-open', 'Keep the headed Chrome instance running after login.', false)
  .action(async (url: string, options) => {
    const port = options.port as number;
    const profileDir = options.profileDir as string;
    const chromePath = options.chromePath as string;
    const prewarmTimeout = options.prewarmTimeout as number;
    ...
    try {
      ...
      console.log('');
      console.log(`Complete login for ${targetUrl.hostname} in the opened Chrome window.`);
      const rl = readline.createInterface({ input, output });
      await rl.question('Press Enter once you are done...');
      rl.close();
      ...
      const authCheck = await needsLoginForUrl(page, targetUrl);
      if (!authCheck.needsLogin) {
        await markHostVerified(resolvedProfileDir, targetUrl.hostname);
        console.log(`✓ Login looks good for ${targetUrl.hostname}`);
      } else {
        console.warn(`! Login may still be required (current URL: ${authCheck.finalUrl})`);
      }
    } finally {
      await browser.disconnect();
    }
    ...
    if (keepOpen) {
      console.log(`✓ Leaving Chrome running on http://localhost:${port}`);
      return;
    }
    ...
    console.log(`✓ Headless Chrome listening on http://localhost:${port}`);
  });

program
  .command('ensure-auth <url>')
  .description('Check whether the CDP profile can access a URL; prompts you to login if needed.')
  .option('--port <number>', 'Debugger port (default: 9222)', (value) => Number.parseInt(value, 10), DEFAULT_PORT)
  .option('--profile-dir <path>', 'Chrome user-data-dir to use.', DEFAULT_PROFILE_DIR)
  .option('--chrome-path <path>', 'Path to the Chrome binary.', DEFAULT_CHROME_BIN)
  .option('--prewarm-timeout <seconds>', 'Policy prewarm timeout in seconds (default: 20).', (value) => Number.parseInt(value, 10), 20)
  .option('--timeout <seconds>', 'Navigation timeout in seconds (default: 15).', (value) => Number.parseInt(value, 10), 15)
  .action(async (url: string, options) => {
    const port = options.port as number;
    const profileDir = options.profileDir as string;
    const chromePath = options.chromePath as string;
    const prewarmTimeout = options.prewarmTimeout as number;
    const timeoutSeconds = options.timeout as number;
    ...
    try {
      const page = await browser.newPage();
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout: Math.max(1, timeoutSeconds) * 1000 });
      const authCheck = await needsLoginForUrl(page, targetUrl);
      if (authCheck.needsLogin) {
        console.log(`✗ Login required for ${targetUrl.hostname}`);
        console.log(`  Current URL: ${authCheck.finalUrl}`);
        console.log('  Run:');
        console.log(
          `    ${invocationBase()} login ${shellQuote(url)} --profile-dir ${shellQuote(resolvedProfileDir)} --port ${port}`,
        );
        process.exitCode = 1;
      } else {
        await markHostVerified(resolvedProfileDir, targetUrl.hostname);
        console.log(`✓ Auth OK for ${targetUrl.hostname}`);
      }
